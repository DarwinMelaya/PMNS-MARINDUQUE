             ...
             <div class="container">
                  <div class="card-body">
                      <div class="track">
                          ...
                          <div class="step <?php if($pidocs=='24'){echo 'active';} ?>"> <span class="icon"> <i class="fa fa-thumbs-up"></i> </span> <span class="text"> Project Implementation</span> </div>
                          <div class="step <?php if($pidocs=='24' && $progress>0){echo 'active';} ?>"> <span class="icon"> <i class="fa fa-eye"></i> </span> <span class="text"> Project Monitoring</span> </div>
                          <div class="step <?php if($pidocs=='24' && $progress>2 && $terminal=='2'){echo 'active';} ?>"> <span class="icon"> <i class="fa fa-check"></i> </span> <span class="text"> Project Completion </span> </div>
                ...
                <div class="info-box bg-lime">
                  <div class="info-box-content">
                    ...
                    <span class="info-box-number text-center mb-0">
                      <?php
                      if($realigned_budget == 'yes'){ echo 'PhP '. number_format(($realigned_counterpart+$realigned_ps+$realigned_mooe+$realigned_eo),2).' (realigned)';}
                      else{ echo 'PhP '.number_format(($ps+$mooe+$eo), 2); }
                      ?>
                    </span>
                  </div>
                </div>
              ...
              <div class="col-12">
                ...
                <table class="table table-bordered table-striped">
                     <tbody>
                        ...
                        <tr>
                          ...
                          <?php if($revised_duration =='yes'){echo '<b>(Revised Duration)</b>';} ?>
                        </td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                          ...
                          <td><?php if($mode_pro=="1"){echo "Direct Release"; }else{ echo "Regional Office Procurement"; } ?></td>
                        </tr>
                        </tbody>
                </table>
                <hr />
                ...
                <thead>
                  <tr>
                    ...
                    <th>As Approved</th>
                    <?php if($realigned_budget == 'yes'){ echo '<th>Realignment</th>'; } ?>
                  </tr>
                </thead>
                ...
                <tbody>
                    ...
                    <tr>
                            ...
                            <?php if($realigned_budget == 'yes'){ echo '<td>Php '.number_format($realigned_ps,2).'</td>'; } ?>
                    </tr>
                    <tr>
                            <th class="text-nowrap" scope="row">Maintenance and Other Expense</th>
                            <td>Php <?= number_format($mooe, 2) ?></td>
                            <?php if($realigned_budget == 'yes'){ echo '<td>Php '.number_format($realigned_mooe,2).'</td>'; } ?>
                    </tr>
                    <tr>
                            <th class="text-nowrap" scope="row">Equipment Outlay</th>
                            <td>Php <?= number_format($eo, 2) ?></td>
                            <?php if($realigned_budget == 'yes'){ echo '<td> Php '.number_format($realigned_eo,2).'</td>'; } ?>
                    </tr>
                    <tr>
                            <th class="text-nowrap" scope="row">Counterpart Funding</th>
                            <td>Php <?= number_format($counterpart_fund, 2) ?></td>
                            <?php if($realigned_budget == 'yes'){ echo '<td> Php '.number_format($realigned_counterpart,2).'</td>'; } ?>
                    </tr>
                    <tr>
                            <th class="text-nowrap" scope="row">Amount of Assistance</th>
                            <td>Php <?= number_format(($ps+$mooe+$eo), 2) ?></td>
                            <?php if($realigned_budget == 'yes'){ echo '<td> Php '.number_format(($realigned_ps+$realigned_mooe+$realigned_eo),2).'</td>'; } ?>
                    </tr>

                    <tr>
                            <th class="text-nowrap" scope="row">Total Project Cost</th>
                            <td>Php <?= number_format(($ps+$mooe+$eo+$counterpart_fund), 2) ?></td>
                            <?php if($realigned_budget == 'yes'){ echo '<td> Php '.number_format(($realigned_counterpart+$realigned_ps+$realigned_mooe+$realigned_eo),2).'</td>'; } ?>
                    </tr>
        ...
        <form action="../backend/save_pids_uploads.php" method="post" enctype="multipart/form-data" id="uploadForm">
          <div class="form-group">
            ...
            <select class="form-control" name="documentTitle" required>
              ...
              <?php
                include '../../connection/connection.php';
                $sql = "SELECT d.doc_id as docs, document_title FROM docs_list d LEFT JOIN pidocs_uploads p ON d.doc_id = p.doc_id AND p.project_id = '".$_GET['id']."' WHERE d.setup_req = '1' and p.doc_id IS NULL order by d.setup_arr asc";
                $result = $conn->query($sql);
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
